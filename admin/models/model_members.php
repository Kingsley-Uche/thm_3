...
Class Model_members extends Database {
    public $conn;
                ...
                function save_member($f_name,$l_name,$year,$month,$day,$email,$address,$phone){
                        ...
                        $stmt =$this->conn->prepare("INSERT INTO members (f_name, l_name, email,phone, address,year_dob,month_dob,day_dob) VALUES(?,?,?,?,?,?,?,?)");
                        $stmt ->bind_param("ssssssss",$f_name,$l_name,$email,$phone,$address,$year,$month,$day);
                       $status= $stmt->execute();
                       $info=[];
                       if($status==1){
                        
                         $info ['update']= "<div class='alert alert-success'>Member saved successfully</div>";
                         $info['goto_page']='id=member/fetch_members';
  $_SESSION['update']="<div class='alert alert-success'>Member saved successfully</div>";
...
return;
  ...
  public function member_update($data){
    ...
    $f_name = mysqli_real_escape_string($this->conn,$data['fname']);
        ...
        $id =mysqli_real_escape_string($this->conn,$data['member_update']);    
     ...
     $sql="UPDATE members SET f_name='$f_name',l_name='$l_name',email='$email',phone='$phone',address='$address',id='$id',year_dob='$year',month_dob='$month',day_dob='$day' WHERE members.id='$id'";
  ...
  function member_exist($phone,$email){
   ...
   $stmt =$this->conn->prepare("SELECT* FROM members WHERE phone=? OR email=?");
