...
Class Model_follow_up extends Database {
    public $conn;
    ...
    function save_member($f_name,$l_name,$year,$month,$day,$email,$address,$phone){
      ...
      $stmt =$this->conn->prepare("INSERT INTO follow_up (f_name, l_name, email,phone, address,year_dob,month_dob,day_dob) VALUES(?,?,?,?,?,?,?,?)");
...
$info['goto_page']='id=follow_up/fetch_follow_up';
...
public function member_edit($id){
  $sql="SELECT* FROM follow_up WHERE follow_up.id='$id'";
  $sta=$this->conn->query($sql);
...
public function delete_member($id){
  ...
  $stmt =$this->conn->prepare("DELETE FROM follow_up WHERE id=?");
...
return;
 ...
 public function member_update($data){
  ...
  $f_name = mysqli_real_escape_string($this->conn,$data['fname']);
      ...
      $id =mysqli_real_escape_string($this->conn,$data['follow_up_update']);    
   ...
   $sql="UPDATE follow_up SET f_name='$f_name',l_name='$l_name',email='$email',phone='$phone',address='$address',id='$id',year_dob='$year',month_dob='$month',day_dob='$day' WHERE follow_up.id='$id'";
...
echo json_encode($info);
  ...
  function member_exist($phone,$email){
   ...
   $stmt =$this->conn->prepare("SELECT* FROM follow_up WHERE phone=? AND email=?");
